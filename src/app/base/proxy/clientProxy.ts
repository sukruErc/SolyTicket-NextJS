...
export class ClientProxy {
  ...
  private async callById<TResponse>(serviceCallTypeEnum: ServiceCallTypeEnum, operationName: string, id?: string): Promise<TResponse> {
    const url = this.getQueryUrl(id);

    return await ServiceProxy.Call<TResponse>(serviceCallTypeEnum, operationName, this.getClientProps({ url })).then((response: any) => {
  ...
  private async callByRequest<TResponse>(serviceCallTypeEnum: ServiceCallTypeEnum, url: string, operationName: string, request?: any): Promise<TResponse> {
    return await ServiceProxy.Call<TResponse>(serviceCallTypeEnum, operationName, this.getClientProps({ url }), request).then((response: any) => {
      return response?.data?.Data;
    }).catch(async (err) => {
      const response: IException = handleException(err);
